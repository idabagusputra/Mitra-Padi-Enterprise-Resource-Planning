...
<script>
    ...
    // ============================================
// GLOBAL SMART FORMAT FOR DATABASE VALUES
// ============================================
function smartFormatFromDB(value) {
    if (value === null || value === undefined || value === '') return '';

    const num = parseFloat(value);
    if (isNaN(num)) return value;

    // Jika bulat (.00), hilangkan desimal
    if (num % 1 === 0) {
        return num.toString();
    }

    // Jika desimal, hilangkan trailing zero
    // Contoh: 10.50 -> 10.5, 10.25 -> 10.25
    return num.toString().replace(/\.?0+$/, '');
}

// ============================================
// AUTO-APPLY TO ALL AUTOFILL FUNCTIONS
// ============================================
// Wrap existing autoFillBeras
const originalAutoFillBeras = autoFillBeras;
window.autoFillBeras = function(beras) {
    if (!beras) return;

    const fields = {
        'giling_kotor': smartFormatFromDB(beras.giling_kotor),
        'jemur': smartFormatFromDB(beras.jemur),
        'pinjam': smartFormatFromDB(beras.pinjaman_beras),
        'pulang': smartFormatFromDB(beras.beras_pulang),
        'harga_jual': smartFormatFromDB(beras.harga)
    };
...
// Wrap existing autoFillKongaMenir
const originalAutoFillKongaMenir = autoFillKongaMenir;
window.autoFillKongaMenir = function(konga_menir) {
    if (!konga_menir) return;

    const fields = {
        'jumlah_konga': smartFormatFromDB(konga_menir.total_konga),
        'harga_konga': '',
        'jumlah_menir': smartFormatFromDB(konga_menir.total_menir),
        'harga_menir': ''
    };
    ...
    const totalKarungKonga = smartFormatFromDB(konga_menir.total_karung_konga) || 0;
    if (totalKarungKonga > 0) {
        addPengambilanKarungKonga(totalKarungKonga);
    }
};

// Wrap existing addPengambilanKarungKonga
const originalAddPengambilanKarungKonga = addPengambilanKarungKonga;
window.addPengambilanKarungKonga = function(jumlah) {
    const formattedJumlah = smartFormatFromDB(jumlah);
    pengambilanCount++;
    const newPengambilan = `
        <div class="pengambilan-item row mb-2">
            ...
            <div class="col-md-3">
                <div class="form-group mb-0">
                    <input type="text" name="pengambilans[${pengambilanCount}][jumlah]" class="form-control number-format pengambilan-w" placeholder="Jumlah" inputmode="numeric" data-raw-value="${formattedJumlah}" value="${formattedJumlah}">
                </div>
            </div>
            <div class="col-md-3">
                <div class="form-group mb-0">
