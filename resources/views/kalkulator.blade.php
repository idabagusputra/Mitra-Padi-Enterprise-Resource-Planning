...
<div class="table-container2">
    ...
    <button class="btn btn-print half" onclick="saveRataRataHargaGabah('jumlah')" style="background-color: #0097A7; text-align: center; margin: 0 !important;">
...
<button class="btn btn-print half" onclick="saveRataRataHarga('jumlah')" style="background-color: #0097A7; text-align: center; margin: 0 !important;">
...
<button class="btn btnnn btn-save half" onclick="printLangsungRata('jumlah')"
    ...
    <i class="fas fa-file-invoice-dollar"></i>
        ...
        <button class="btn btn-print half" onclick="saveNotaAsJPG('jumlah')" style="text-align: center; margin: 0 !important;">
...
if (isIOS) {
    await shareNotaToWhatsApp_iOS(notaContent, calculatorType);
    return;
}
...
async function shareNotaToWhatsApp_iOS(notaContent, calculatorType) {
    try {
        const iframe = document.createElement('iframe');
        iframe.style.position = 'absolute';
        iframe.style.left = '-9999px';
...
const blob = await new Promise(resolve =>
    finalCanvas.toBlob(resolve, 'image/jpeg', 1.0)
        ...
        // === Cek apakah Web Share API support file sharing ===
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
            ...
            if (newWindow) {
                newWindow.document.write(`
                    ...
                    <body>
                        <p class="info">Tekan lama gambar → "Simpan ke Foto", lalu kirim via WhatsApp.</p>
                        <img src="${imgData}" alt="Nota">
                    </body>
                    </html>
                `);
        ...
        if (error.name !== 'AbortError') {
            // AbortError = user cancel share sheet, bukan error
            console.error('Error sharing on iOS:', error);
            alert('Gagal membagikan nota. Silakan coba lagi.');
        }
    }
}
...
<body>
  ...
  <div style="font-size:11.5px;font-weight:900;margin-bottom:1.5mm;">Cara Hitung Rata" Harga Jual Beras :</div>
