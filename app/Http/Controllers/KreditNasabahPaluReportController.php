...
class KreditNasabahPaluReportController extends Controller
{

    public function index()
    {
        // Ambil 1 data terbaru berdasarkan id terbesar
        $rekapanKreditTerbaru = RekapKreditNasabahPalu::orderBy('id', 'desc')->first(); // Mengambil 1 data terbaru

        // Menggunakan paginate() untuk mengambil 20 data per halaman
        $rekapanKredits = RekapKreditNasabahPalu::orderBy('id', 'desc')->paginate(20);

        // Mengirim data terbaru dan data lainnya ke view
        return view('daftar-rekapan-kredit-nasabah-palu', compact('rekapanKredits', 'rekapanKreditTerbaru'));
    }
    ...
    {
        $gilingId = $request->input('gilingId');
        $folderPath = public_path('rekapan_kredit_nasabah_palu');
        Log::info("Folder Path: " . $folderPath);

        // Cari file yang sesuai pola
        $matchingFiles = glob("{$folderPath}/Rekapan_Kredit_Nasabah_Palu_{$gilingId}_*.pdf");
    ...
    public function generatePdf()
    {
        ...
        $now = Carbon::now()->subDays(2)->startOfDay();
        $calculatedKredits = $allKredits->map(function ($kredit) use ($now) {
            ...
            if ($kredit->status === true) {
                ...
                $now = Carbon::now()->subDays(2)->startOfDay(); // Dapatkan waktu sekarang
        ...
        try {
            ...
            return redirect()->route('rekapKreditNasabahPalu.index')
